...
/**
 * The stage that displays the list of connected clients. When the client is RED
 * they haven't voted for this question. If the cleint is GREEN it will list
 * their vote after their ID.
 * 
 * @author Jay
 */
public class ClientStage extends Stage {
	...
	/**
	 * TextFlow that will be used to display the updated client list Uses a 1
	 * second timer to constantly recall the list and redraw
	 */
			...
			Platform.runLater(new Runnable() {
				public void run() {
					...
					for (Client c : session.getClients()) {
						// appendText(c.getID() + "\n");
						Text t = new Text(
								c.getID() + " - " + (c.getLastVote() != null ? c.getLastVote().getID() : "") + "\n");
	 ...
	 * 
	 * @param s
	 *            Reference back to the running session
	 */
